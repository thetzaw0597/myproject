...
function setMessage(id) {
    ...
    if (msgTextElement) {
        ...
        console.error("Element with ID 'msgtext' not found!");
    }
}
...
function isSelect(selectSquare) {
    if (winningLine) return "1"; // If there's already a winner, stop further actions.
...
window.addEventListener("DOMContentLoaded", function () {
    ...
    squareArray.forEach(square => square.classList.add("js-clickable"));

    squareArray.forEach(square => {
        square.addEventListener('click', () => {
            let gameOverFlg = isSelect(square);
            if (gameOverFlg === "0" && flag === "bear-flag") {
